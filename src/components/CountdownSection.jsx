...
function CountdownCard({ value, label, delay }) {
  ...
  return (
    ...
    >
      {/* Number box */}
      <div
        className="relative w-20 h-24 sm:w-24 sm:h-28 md:w-28 md:h-32 rounded-2xl flex items-center justify-center overflow-hidden"
        style={{
      ...
      >
        ...
        <div
          className="absolute inset-x-0 top-1/2 h-[1px] pointer-events-none"
        ...
        <AnimatePresence mode="popLayout">
          <motion.span
            ...
            className="font-serif text-4xl sm:text-5xl md:text-5xl font-light text-gradient-gold select-none"
            aria-live="polite"
            aria-atomic="true"
          >
            {pad(value)}
      ...
      {/* Label */}
      <p
        className="mt-3 font-sans text-[10px] tracking-[0.38em] uppercase"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-hidden="true"
        style={{
          background: 'radial-gradient(ellipse 70% 50% at 50% 50%, var(--accent-gold), transparent)',
          opacity: 0.04,
        }}
      />
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        {isPast ? (
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="font-script text-5xl text-gradient-gold"
          >
        ...
        <motion.p
          ...
          className="mt-14 font-serif italic text-lg"
          style={{ color: 'var(--text-muted)' }}
          aria-label={`Date du mariage : ${config.wedding.date} ${config.wedding.year}`}
        >
