...
function MoroccanStarSmall() {
  return (
    <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="32" height="32">
      <path d="M20 2 L23.6 13.2 L35 9.6 L27.2 18.6 L35 27.6 L23.6 24 L20 35 L16.4 24 L5 27.6 L12.8 18.6 L5 9.6 L16.4 13.2 Z" fill="var(--accent-gold)" fillOpacity="0.35" stroke="var(--accent-gold)" strokeWidth="0.6" strokeOpacity="0.6" />
...
export default function FooterSection() {
  ...
  return (
    <footer style={{ background: 'linear-gradient(180deg, var(--bg-primary) 0%, var(--bg-secondary) 100%)', borderTop: '1px solid var(--border-color)' }} aria-label="Pied de page">
      <section id="closing" className="section-padding px-6">
        <div className="max-w-5xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, scale: 0.5, rotate: -20 }} whileInView={{ opacity: 1, scale: 1, rotate: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="flex justify-center mb-8">
            <MoroccanStarSmall />
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1, duration: 0.85, ease: [0.16, 1, 0.3, 1] }} className="mx-auto w-full max-w-5xl px-2 py-7 sm:px-10 sm:py-8">
            <h2 className="font-script name-safe whitespace-nowrap text-gradient-gold leading-[1.2]" style={{ fontSize: 'clamp(2.2rem, 10vw, 5rem)' }}>{couple.combined}</h2>
          </motion.div>
